...
function randomMobileNumGenInStringFormat(x)
{
    var tenMultiplier = Math.pow(10, x)
    var randNo,mostSigDigit= 0;
    //while(continuIter){
...
function convertJsonArray(jsonArray){
    ...
    for(var index=0; index<jsonArray.length; index++){
        data.push({"name": jsonArray[index]})
    }
    print (data)
...
function convertJsonArrayOfChallengeAns(jsonArray){
    var data = new Array();
    for(var index=0; index<jsonArray.length; index++){
        data.push({"questionNumber": index,"answer": jsonArray[index]})
    }
    print (data)
    jsonStr = JSON.stringify(data);
    return jsonStr
